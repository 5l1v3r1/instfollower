...
import com.airbnb.lottie.C1265L;
...
public abstract class BaseStrokeContent implements BaseKeyframeAnimation.AnimationListener, KeyPathElementContent, DrawingContent {
    ...
    /* renamed from: pm */
    private final PathMeasure f274pm;
    ...
    public void draw(Canvas canvas, Matrix matrix, int i) {
        ...
        int parentAlpha = i;
        C1265L.beginSection("StrokeContent#draw");
        if (Utils.hasZeroScaleAxis(parentMatrix)) {
            float endSection = C1265L.endSection("StrokeContent#draw");
            return;
        }
        ...
        if (this.paint.getStrokeWidth() <= 0.0f) {
            float endSection2 = C1265L.endSection("StrokeContent#draw");
            return;
        }
        ...
        for (int i2 = 0; i2 < this.pathGroups.size(); i2++) {
            ...
            if (pathGroup.trimPath != null) {
                ...
                C1265L.beginSection("StrokeContent#buildPath");
                this.path.reset();
                for (int j = pathGroup.paths.size() - 1; j >= 0; j--) {
                    this.path.addPath(((PathContent) pathGroup.paths.get(j)).getPath(), parentMatrix);
                }
                float endSection3 = C1265L.endSection("StrokeContent#buildPath");
                C1265L.beginSection("StrokeContent#drawPath");
                canvas2.drawPath(this.path, this.paint);
                float endSection4 = C1265L.endSection("StrokeContent#drawPath");
            }
        }
        float endSection5 = C1265L.endSection("StrokeContent#draw");
    }
    ...
    private void applyTrimPath(Canvas canvas, PathGroup pathGroup, Matrix matrix) {
        ...
        Matrix parentMatrix = matrix;
        C1265L.beginSection("StrokeContent#applyTrimPath");
        if (pathGroup2.trimPath == null) {
            float endSection = C1265L.endSection("StrokeContent#applyTrimPath");
            return;
        }
        this.path.reset();
        ...
        this.f274pm.setPath(this.path, false);
        float length = this.f274pm.getLength();
        while (true) {
            totalLength = length;
            if (!this.f274pm.nextContour()) {
                break;
            }
            length = totalLength + this.f274pm.getLength();
        }
        ...
        for (int j2 = pathGroup2.paths.size() - 1; j2 >= 0; j2--) {
            ...
            this.f274pm.setPath(this.trimPathPath, false);
            float length2 = this.f274pm.getLength();
        ...
        float endSection2 = C1265L.endSection("StrokeContent#applyTrimPath");
    }
    ...
    public void getBounds(RectF rectF, Matrix matrix, boolean z) {
        ...
        Matrix parentMatrix = matrix;
        boolean z2 = z;
        C1265L.beginSection("StrokeContent#getBounds");
        this.path.reset();
        ...
        float endSection = C1265L.endSection("StrokeContent#getBounds");
    }

    private void applyDashPatternIfNeeded(Matrix matrix) {
        PathEffect pathEffect;
        Matrix parentMatrix = matrix;
        C1265L.beginSection("StrokeContent#applyDashPattern");
        if (this.dashPatternAnimations.isEmpty()) {
            float endSection = C1265L.endSection("StrokeContent#applyDashPattern");
            return;
        }
        ...
        float endSection2 = C1265L.endSection("StrokeContent#applyDashPattern");
    }
    ...
    private static final class PathGroup {
        ...
        /* synthetic */ PathGroup(TrimPathContent x0, C13011 r7) {
            this(x0);
            C13011 r2 = r7;
        }
