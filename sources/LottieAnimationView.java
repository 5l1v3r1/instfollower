...
public class LottieAnimationView extends AppCompatImageView {
    ...
    private void init(@Nullable AttributeSet attrs) {
        ...
        String url;
        Throwable th;
        TypedArray ta = getContext().obtainStyledAttributes(attrs, C0551R.styleable.LottieAnimationView);
        if (!isInEditMode()) {
            boolean hasRawRes = ta.hasValue(C0551R.styleable.LottieAnimationView_lottie_rawRes);
            boolean hasFileName = ta.hasValue(C0551R.styleable.LottieAnimationView_lottie_fileName);
            boolean hasUrl = ta.hasValue(C0551R.styleable.LottieAnimationView_lottie_url);
            if (hasRawRes && hasFileName) {
                ...
                int rawResId = ta.getResourceId(C0551R.styleable.LottieAnimationView_lottie_rawRes, 0);
                if (rawResId != 0) {
                    setAnimation(rawResId);
                }
            } else if (hasFileName) {
                String fileName = ta.getString(C0551R.styleable.LottieAnimationView_lottie_fileName);
                if (fileName != null) {
                    setAnimation(fileName);
                }
            } else if (hasUrl && (url = ta.getString(C0551R.styleable.LottieAnimationView_lottie_url)) != null) {
                setAnimationFromUrl(url);
            }
        }
        if (ta.getBoolean(C0551R.styleable.LottieAnimationView_lottie_autoPlay, false)) {
            this.wasAnimatingWhenDetached = true;
            this.autoPlay = true;
        }
        if (ta.getBoolean(C0551R.styleable.LottieAnimationView_lottie_loop, false)) {
            this.lottieDrawable.setRepeatCount(-1);
        }
        if (ta.hasValue(C0551R.styleable.LottieAnimationView_lottie_repeatMode)) {
            setRepeatMode(ta.getInt(C0551R.styleable.LottieAnimationView_lottie_repeatMode, 1));
        }
        if (ta.hasValue(C0551R.styleable.LottieAnimationView_lottie_repeatCount)) {
            setRepeatCount(ta.getInt(C0551R.styleable.LottieAnimationView_lottie_repeatCount, -1));
        }
        if (ta.hasValue(C0551R.styleable.LottieAnimationView_lottie_speed)) {
            setSpeed(ta.getFloat(C0551R.styleable.LottieAnimationView_lottie_speed, 1.0f));
        }
        setImageAssetsFolder(ta.getString(C0551R.styleable.LottieAnimationView_lottie_imageAssetsFolder));
        setProgress(ta.getFloat(C0551R.styleable.LottieAnimationView_lottie_progress, 0.0f));
        enableMergePathsForKitKatAndAbove(ta.getBoolean(C0551R.styleable.LottieAnimationView_lottie_enableMergePathsForKitKatAndAbove, false));
        if (ta.hasValue(C0551R.styleable.LottieAnimationView_lottie_colorFilter)) {
            new SimpleColorFilter(ta.getColor(C0551R.styleable.LottieAnimationView_lottie_colorFilter, 0));
            new KeyPath("**");
            new LottieValueCallback(filter);
            addValueCallback(keyPath, LottieProperty.COLOR_FILTER, lottieValueCallback);
        }
        if (ta.hasValue(C0551R.styleable.LottieAnimationView_lottie_scale)) {
            this.lottieDrawable.setScale(ta.getFloat(C0551R.styleable.LottieAnimationView_lottie_scale, 1.0f));
        }
        ta.recycle();
    ...
    public void setMaxProgress(@FloatRange(from = 0.0d, mo103to = 1.0d) float endProgress) {
    ...
    public void setMinAndMaxProgress(@FloatRange(from = 0.0d, mo103to = 1.0d) float minProgress, @FloatRange(from = 0.0d, mo103to = 1.0d) float maxProgress) {
    ...
    public void setProgress(@FloatRange(from = 0.0d, mo103to = 1.0d) float progress) {
    ...
    private static class SavedState extends View.BaseSavedState {
        ...
        /* synthetic */ SavedState(Parcel x0, C12661 r7) {
            this(x0);
            C12661 r2 = r7;
        }
